...
export default function GrindHatPage() {
  ...
  const getNftImageUrl = async (collection: typeof collections[CollectionName], id: string): Promise<string> => {
    ...
    try {
      // Get NFT data from Alchemy via API route
      ...
      // Priority 1: Use pngUrl if available (best quality and most reliable)
      if (alchemyData.image && alchemyData.image.pngUrl) {
        imageUrl = alchemyData.image.pngUrl;
      }
      // Priority 2: Use cachedUrl if available
      else if (alchemyData.image && alchemyData.image.cachedUrl) {
        imageUrl = alchemyData.image.cachedUrl;
      }
      // Priority 3: Use originalUrl if available
      else if (alchemyData.image && alchemyData.image.originalUrl) {
        imageUrl = alchemyData.image.originalUrl;
      }
      // Priority 4: Use raw.metadata.image
      else if (alchemyData.raw && alchemyData.raw.metadata && alchemyData.raw.metadata.image) {
        imageUrl = alchemyData.raw.metadata.image;
      }
      // Priority 5: Use thumbnailUrl as last resort
